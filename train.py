...
if __name__ == '__main__':
    ...
    #checkpoints dir
    opt.checkpoint_dir= os.path.join(opt.path,opt.project_name)
    ...
    writer = SummaryWriter(
        '.tmp/log/' + opt.dataset + '_' + opt.backbone + '_' + opt.mode + '_' + f'{datetime.datetime.now().strftime("%Y%m%d-%H%M%S")}')
    ...
    if opt.dataset == 'cdd':
        opt.dataset_dir = '../CDDataset/ChangeDetection/'
    elif opt.dataset == 'levir':
        opt.dataset_dir = '../CDDataset/LEVIR/'
    elif opt.dataset_dir == 'levir+':
        opt.dataset_dir = '../CDDataset/LEVIR-CD+_256'
    ...
    if opt.backbone == 'resnet':
        optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.99,
                                    weight_decay=0.0005)  # Be careful when you adjust learning rate, you can refer to the linear scaling rule
